...
import { VisuallyHiddenInput } from "./visually-hidden-input.tsx";
...
const colorFormats = ["hex"] as const;
...
function ColorPickerContent(
  ...
  return (
    <PopoverContent
      ...
      {...popoverContentProps}
      className={cn(
        "flex w-85 border border-gray-200 flex-col gap-4 p-4",
        className,
      )}
    >
      {children}
...
function ColorPickerArea(props: DivProps) {
  ...
  return (
    <AreaPrimitive
      ...
      {...areaProps}
      className={cn(
        "relative h-40 w-full cursor-crosshair touch-none rounded-sm",
...
function ColorPickerSwatch(props: DivProps) {
  ...
  return (
    <SwatchPrimitive
      ...
      {...swatchProps}
      className={cn(
        "box-border size-8 rounded-sm shadow-sm",
...
function ColorPickerEyeDropper(props: React.ComponentProps<typeof Button>) {
  ...
  return (
    ...
    >
      {children ?? <PipetteIcon className="w-4 h-4" />}
    </Button>
  );
}
